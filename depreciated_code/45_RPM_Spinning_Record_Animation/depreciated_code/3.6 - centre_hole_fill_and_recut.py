"""
centre_hole_fill_and_recut.py - Version 3.6
...
New in 3.6:
- Fixes cv2.floodFill error handling and return value unpacking
- Ensures image is contiguous in memory for proper floodFill operation
- Validates seed point is within bounds before floodFill
- Corrects newVal format for 3-channel BGR image
...
def fill_and_recut_center_hole(
    ...
    try:
        print("=" * 60)
        print("FILL AND RECUT CENTER HOLE AND VINYL BODY - VERSION 3.6 (OpenCV)")
        print("=" * 60)
        ...
        if img_bgra.shape[2] == 4:
            bgr = img_bgra[:, :, :3].copy()  # Make a copy to ensure contiguity
        ...
        # STEP 3: Fill center hole with floodFill
        ...
        if not (0 <= center_x < width and 0 <= center_y < height):
            result['message'] = f"ERROR: Seed point ({center_x}, {center_y}) is out of bounds ({width}x{height})"
            print(result['message'])
            return result

        # Ensure newVal is properly formatted as integer tuple for BGR image
        new_val = tuple(int(c) for c in edge_color)

        # Perform floodfill from center point with edge color
        # cv2.floodFill returns: retval (int) - number of pixels filled
        try:
            ...
            print(f"Center hole filled with color {new_val}")
        ...
        # Split filled BGR image into channels
        b, g, r = cv2.split(img_filled)

        # Merge BGR channels with combined alpha
        ...
        print(f"Edge color found:   {new_val} (BGR format)")
        print(f"Fill operation:     FloodFill from center")
        print(f"Pixels filled:      {num_filled}")
...
if __name__ == '__main__':
    print("\n" + "=" * 60)
    print("VINYL RECORD CENTER HOLE FILL AND RECUT - VERSION 3.6 (OpenCV)")
    print("=" * 60)
